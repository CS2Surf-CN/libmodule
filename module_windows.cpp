...
bool CModule::Init(const std::string_view svModelePath) {
	...
	for (WORD i = 0; i < pNTHeaders->FileHeader.NumberOfSections; ++i) // Loop through the sections.
	{
		...
		m_vModuleSections.emplace_back(reinterpret_cast<const char*>(hCurrentSection.Name),
									   static_cast<uintptr_t>(reinterpret_cast<uintptr_t>(handle) + hCurrentSection.VirtualAddress),
...
CMemory CModule::GetVirtualTableByName(const std::string_view svTableName, bool bDecorated) const {
	...
	const uintptr_t rttiTDRva =
		rttiTypeDescriptor - GetModuleBase(); // The RTTI gets referenced by a 4-Byte RVA address. We need to scan for that address.
